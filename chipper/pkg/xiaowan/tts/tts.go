...
func OpenAItts(outputFile string, aiText string) {
	...
	if _, err = os.Stat(outputFile); err == nil {
		...
		if err != nil {
			// 删除文件时出错
			logger.Println("Failed to delete existing file %s: %s", outputFile, err)
			return
		}
		logger.Println("Existing file %s deleted successfully.\n", outputFile)
	} else if !os.IsNotExist(err) {
		// 访问文件时出现了其他错误
		logger.Println("Error checking file %s: %s", outputFile, err)
		return
	}
